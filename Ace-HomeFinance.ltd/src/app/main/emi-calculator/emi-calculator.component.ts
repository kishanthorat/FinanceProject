...
import { Chart, Slider } from 'app/model/section';
  ...
  */
  calculateEMI() {
    if(this.loanAmount && this.intRate && this.loanDuration) {
      ...
      let emi = this.loanAmount * mir * Math.pow((1 + mir), this.loanDuration*12) / ((Math.pow((1 + mir), this.loanDuration*12)) - 1);
      let totalPayment = emi * this.loanDuration*12;
